...
import { Check, ChevronDown, Delete, Plus, X } from 'lucide-vue-next'
...
function pressKey(key: string) {
  ...
  if (rawAmount.value.length >= 10) return // Max 10 digits (~9.9 billion IDR)
...
function onKeydown(e: KeyboardEvent) {
  ...
  if (showNoteInput.value) return // Allow typing in description input
...
<template>
  <Teleport to="body">
    ...
    >
      <div
        v-if="open"
        class="fixed inset-x-0 bottom-0 z-50 flex max-h-[92vh] flex-col rounded-t-[28px] border-t border-[#1f222e] bg-[#0a0a0c] text-zinc-100 shadow-2xl safe-bottom"
        @click.stop
      >
        <!-- Drag pill handle -->
        ...
        <div class="flex items-center justify-between px-6 pt-2 pb-1">
          <div class="flex items-center gap-1 rounded-xl bg-[#121217] p-1 border border-[#1f222e]">
            <button
              type="button"
          ...
          <button
            type="button"
            class="flex size-9 items-center justify-center rounded-full bg-zinc-900 text-zinc-400 hover:text-zinc-100 transition-colors"
            @click="close"
          >
            <X class="size-5" />
          </button>
        ...
        <div class="flex flex-col items-center justify-center px-6 py-4">
          <span class="text-xs font-mono uppercase tracking-widest text-zinc-500 mb-1">
          ...
          <div class="flex items-baseline justify-center gap-2">
            ...
            <span
              class="font-mono text-4xl sm:text-5xl font-extrabold tracking-tight tabular-nums"
              :class="
                numericAmount > 0
                  ? type === 'expense' ? 'text-rose-400 text-glow-rose' : 'text-emerald-400 text-glow-emerald'
                  : 'text-zinc-600'
              "
            >
              {{ formattedDisplay }}
            </span>
            <span class="animate-pulse font-mono text-3xl font-light text-emerald-400">_</span>
          </div>
          ...
          <div class="mt-2 flex items-center justify-center">
            ...
            <div v-else-if="!showNoteInput && description" class="flex items-center gap-2">
              <span class="text-xs text-emerald-400 italic font-mono bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-500/20">
                "{{ description }}"
              </span>
              <button
                type="button"
            ...
            <button
              ...
              :class="
                selectedCategoryId === cat.id
                  ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.25)]'
                  : 'bg-[#181820] text-zinc-400 border border-[#1f222e] hover:text-zinc-200'
              "
        ...
        <div class="flex items-center justify-between px-6 py-2.5 border-b border-[#1f222e] bg-[#0a0a0c]">
          <span class="text-xs text-zinc-500 font-mono">Sumber Rekening:</span>
          <div class="relative">
            <button
              type="button"
              class="flex items-center gap-1.5 rounded-lg border border-[#1f222e] bg-[#121217] px-3 py-1.5 text-xs font-mono text-zinc-200 hover:border-emerald-500/50 transition-colors"
              @click="showBalancePicker = !showBalancePicker"
            >
              <span class="font-semibold text-emerald-400">{{ activeBalance?.name || 'Pilih Rekening' }}</span>
              <span v-if="activeBalance" class="text-zinc-500">
                (Rp {{ new Intl.NumberFormat('id-ID').format(activeBalance.final_amount) }})
              </span>
            ...
            <div
              ...
              class="absolute right-0 bottom-full mb-2 z-50 w-56 rounded-xl border border-[#1f222e] bg-[#181820] p-1.5 shadow-2xl"
            >
              <div
                v-for="bal in balances"
                :key="bal.id"
                class="flex items-center justify-between rounded-lg px-3 py-2 text-xs cursor-pointer transition-colors"
                :class="selectedBalanceId === bal.id ? 'bg-emerald-500/20 text-emerald-300' : 'text-zinc-300 hover:bg-zinc-800/60'"
                @click="selectBalance(bal.id)"
              >
                <span class="font-medium">{{ bal.name }}</span>
                <span class="font-mono text-[11px] text-zinc-400">
                  Rp {{ new Intl.NumberFormat('id-ID').format(bal.final_amount) }}
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-3 gap-2 px-6 pt-3 pb-2">
          <button
            v-for="k in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '000', '0', 'BACKSPACE']"
            :key="k"
            type="button"
            class="flex h-13 items-center justify-center rounded-xl border border-[#1f222e] bg-[#121217] text-xl font-mono font-semibold text-zinc-200 transition-all active:scale-95 active:bg-zinc-800 hover:border-zinc-700"
            @click="pressKey(k)"
          >
            <Delete v-if="k === 'BACKSPACE'" class="size-6 text-zinc-400" />
            <span v-else-if="k === '000'" class="text-sm tracking-wider text-emerald-400">000</span>
            <span v-else>{{ k }}</span>
          </button>
        </div>
        ...
        <div class="px-6 pt-2 pb-6">
          <button
            ...
            class="flex h-14 w-full items-center justify-center gap-2 rounded-2xl font-mono text-sm font-bold uppercase tracking-wider transition-all disabled:opacity-40"
            :class="
              type === 'expense'
          ...
          >
            <span v-if="!isSubmitting">
              Simpan {{ type === 'expense' ? 'Pengeluaran' : 'Pemasukan' }} (Rp {{ formattedDisplay }})
            </span>
